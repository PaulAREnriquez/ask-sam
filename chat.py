...
def get_response(msg):
    ...
    X = torch.from_numpy(X).to(dtype=torch.float).to(device) # we convert the numpy array to float tensor [[]]

    output = model.forward(X) # returns a tensor [[]], which is the prediction of the model
    ...
    probs = torch.softmax(output, dim=1) # torch.softmax converts the predictions into probabilities
    
    # probs[0] contains the array of probabilities, and we pass [predicted.item()] as an indexer to get the index of the
    # highest probability
    prob = probs[0][predicted.item()]
    
    if prob.item() > threshold: # we check if the highest probability is greater than our threshold
